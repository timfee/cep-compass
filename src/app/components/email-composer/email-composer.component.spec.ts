...
describe('EmailComposerComponent', () => {
  ...
  beforeEach(async () => {
    const emailServiceSpy = jasmine.createSpyObj('EmailTemplateService', [
      'selectTemplate',
      'setVariableValue',
      'composeEmail',
      ...
      previewSubject: jasmine.createSpy().and.returnValue('Test Subject Test'),
    });
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        { provide: EmailTemplateService, useValue: emailServiceSpy },
      ],
    }).compileComponents();

    emailService = TestBed.inject(EmailTemplateService) as jasmine.SpyObj<EmailTemplateService>;
  ...
  describe('Template Loading', () => {
    ...
    it('should update form when template is loaded', () => {
      ...
      expect(component.emailForm.get('templateId')?.value).toBe('test-template');
    });
  });
  ...
  describe('Recipient Management', () => {
    ...
    it('should not add duplicate recipient email', () => {
      ...
      expect(component.recipientChips().filter(r => r === email).length).toBe(1);
    });
  ...
  describe('Actions', () => {
    ...
    it('should open in Gmail', () => {
      const gmailUrl = 'https://mail.google.com/mail/?view=cm&fs=1&to=test@example.com';
    ...
    it('should compose email when validation passes', () => {
      ...
      expect(component.emailComposed.emit).toHaveBeenCalledWith(mockComposedEmail);
    });
  ...
  describe('Utility Functions', () => {
    it('should validate email format correctly', () => {
      expect((component as unknown as { isValidEmail: (email: string) => boolean }).isValidEmail('test@example.com')).toBeTrue();
      expect((component as unknown as { isValidEmail: (email: string) => boolean }).isValidEmail('invalid-email')).toBeFalse();
      expect((component as unknown as { isValidEmail: (email: string) => boolean }).isValidEmail('test@')).toBeFalse();
      expect((component as unknown as { isValidEmail: (email: string) => boolean }).isValidEmail('@example.com')).toBeFalse();
    });
  ...
  describe('Variable Substitution', () => {
    it('should substitute variables in text', () => {
      ...
      const result = (component as unknown as { substituteVariables: (text: string, values: Record<string, string>) => string }).substituteVariables(text, values);
