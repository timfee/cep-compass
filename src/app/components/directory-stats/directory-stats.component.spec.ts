...
describe('DirectoryStatsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [{ provide: DirectoryService, useValue: spy }],
    }).compileComponents();

    fixture = TestBed.createComponent(DirectoryStatsComponent);
    component = fixture.componentInstance;
    directoryServiceMock = TestBed.inject(
      DirectoryService,
    ...
    directoryServiceMock.searchUsers.and.returnValue(
      Promise.resolve([mockUsers[0]]),
    );
    directoryServiceMock.searchGroups.and.returnValue(
  ...
  it('should call directoryService.loadMoreUsers when load more users button is clicked', async () => {
    ...
    const loadMoreUsersButton = Array.from(buttons).find((btn) =>
      (btn as HTMLButtonElement).textContent?.includes('Load More Users'),
    ) as HTMLButtonElement;
  ...
  it('should call directoryService.loadMoreGroups when load more groups button is clicked', async () => {
    const buttons = fixture.nativeElement.querySelectorAll('button');
    const loadMoreGroupsButton = Array.from(buttons).find((btn) =>
      (btn as HTMLButtonElement).textContent?.includes('Load More Groups'),
    ) as HTMLButtonElement;
